...
public class Database {
	...
	public static String compare(iris.Feature[] gabor, Vector<iris.Feature[]> irisDb, Vector<String> nameDb) { 
		/*
		 * Comparison through Minkowski metric
		 */
		Vector<Float> distance = new Vector<Float>();
		for(iris.Feature[] iris: irisDb) { // iterate through DB
			float dist = 0f;
			for(int i=0; i<iris.length; i++) { // iterate through Gabor factors
				dist = dist + distance(iris[i].toVector(), gabor[i].toVector());  // compute distance
			}
			distance.add(dist);
